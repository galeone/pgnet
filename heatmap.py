...
def main(args):
    ...
    with tf.Graph().as_default() as graph, tf.device(args.device):
        ...
        detect = pgnet.detect(logits, num_classes)
        ...
        # each one with shape [tested_positions, num_classes]
        top_values, top_indices = tf.nn.top_k(
            per_roi_probabilities, k=num_classes)
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            """
            print(sess.run(detect, feed_dict={ "images_:0": input_images }))
            sys.exit(1)
